...
// GlmAsrKvCache: autoregressive KV cache state manager.
...
class GlmAsrKvCache : public GlmAsrInferenceState {
  public:
    ...
    GlmAsrKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, GlmAsrKvCacheNames names = {});
    ...
    // --- GlmAsrKvCache-specific methods (not on the interface) ---
