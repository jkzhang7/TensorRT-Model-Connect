...
def add_activation(
    ...
    """Dispatch activation by name.

    GLM-ASR declares ``hidden_act`` "gelu" and applies the conv front-end
    activation through ``torch.nn.functional.gelu``. Both are the exact,
    erf-based GELU, so "gelu" maps to TensorRT's GELU_ERF here. The tanh
    approximation stays reachable as "gelu_new", matching how Hugging Face
    separates ``ACT2FN["gelu"]`` from ``ACT2FN["gelu_new"]``.
    """
    if activation_type == "gelu":
        return add_gelu_erf(network, inp)
...
# ---------------------------------------------------------------------------
# Rotary position embedding for the fixed-length audio encoder
# ---------------------------------------------------------------------------

def make_encoder_rope_half_tables(
    seq_length: int, rotary_dim: int, rope_theta: float,
) -> tuple[np.ndarray, np.ndarray]:
    """Return (cos, sin) tables of shape [seq_length, rotary_dim // 2].

    The audio encoder attends over a fixed number of frames at positions
    ``arange(seq_length)``, so the tables are constants rather than a runtime
    cache. ``rotary_dim`` is ``head_dim * partial_rotary_factor``: only that
    leading slice of each head rotates, matching ``apply_rotary_pos_emb``.
    """
    half = rotary_dim // 2
    # Accumulate in float64 so the table matches the reference implementation
    # to float32 round-off rather than to float32 accumulation error.
    positions = np.arange(seq_length, dtype=np.float64)[:, None]
    exponents = (2.0 * np.arange(half, dtype=np.float64)) / float(rotary_dim)
    inverse_frequency = rope_theta ** (-exponents)[None, :]
    angles = positions * inverse_frequency
    return np.cos(angles).astype(np.float32), np.sin(angles).astype(np.float32)


def add_apply_rope_rows(
    network: trt.INetworkDefinition,
    rows: trt.ITensor,
    seq_length: int,
    num_heads: int,
    ...
    *,
    ...
    """Apply rotary embedding to ``[seq_length, num_heads*head_dim]`` rows.

    The rows are viewed as ``[seq_length, num_heads, head_dim]`` so one shared
    cos/sin pair rotates every head, then reshaped back to the row layout the
    surrounding matmuls expect. Only the leading ``rotary_dim`` entries of each
    head rotate; the remaining ``head_dim - rotary_dim`` pass through
    unchanged, which is what ``partial_rotary_factor`` below 1.0 means.
    """
    half = rotary_dim // 2

    heads = network.add_shuffle(rows)
    heads.reshape_dims = (seq_length, num_heads, head_dim)
    x = heads.get_output(0)

    first = network.add_slice(
        x, start=(0, 0, 0), shape=(seq_length, num_heads, half), stride=(1, 1, 1),
    ).get_output(0)
    second = network.add_slice(
        x, start=(0, 0, half), shape=(seq_length, num_heads, half), stride=(1, 1, 1),
    ).get_output(0)
    passthrough = None
    if rotary_dim < head_dim:
        passthrough = network.add_slice(
            x, start=(0, 0, rotary_dim),
            shape=(seq_length, num_heads, head_dim - rotary_dim),
            stride=(1, 1, 1),
        ).get_output(0)

    cos = add_constant(
        network, (seq_length, 1, half),
        np.ascontiguousarray(cos_table.reshape(seq_length, 1, half)), dtype=dtype,
    )
    sin = add_constant(
        network, (seq_length, 1, half),
        np.ascontiguousarray(sin_table.reshape(seq_length, 1, half)), dtype=dtype,
    )

    def _mul(a, b):
        return network.add_elementwise(
            a, b, trt.ElementWiseOperation.PROD).get_output(0)

    def _combine(a, b, op):
        return network.add_elementwise(a, b, op).get_output(0)

    rotated_first = _combine(
        _mul(first, cos), _mul(second, sin), trt.ElementWiseOperation.SUB)
    rotated_second = _combine(
        _mul(second, cos), _mul(first, sin), trt.ElementWiseOperation.SUM)

    parts = [rotated_first, rotated_second]
    if passthrough is not None:
        parts.append(passthrough)
    joined = network.add_concatenation(parts)
    joined.axis = 2

    flat = network.add_shuffle(joined.get_output(0))
    flat.reshape_dims = (seq_length, num_heads * head_dim)
